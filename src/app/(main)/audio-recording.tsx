...
export default function AudioRecording() {
  ...
  const audioBufferToFile = async (audioBuffers: AudioBuffer[]): Promise<any> => {
    try {
      ...
      const tempFilePath =
        (FileSystem.cacheDirectory || FileSystem.documentDirectory) + 'temp_recording.wav';
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      recorderRef.current?.onAudioReady((event) => {
        ...
        // Update duration
        const totalDuration = audioBuffersRef.current.reduce(
          (total, buf) => total + buf.duration,
          0
        );
    ...
    <>
      ...
      <View
        ...
        <View className="flex-1 px-6 pt-20">
          ...
          <View className="mb-8 flex-1 items-center justify-center">{/* TODO Waveform */}</View>
          ...
          {/* Timer */}
          <View className="mb-8">
            <Text className="text-center font-mono text-lg text-muted-foreground">
              {formatTime(duration)}
            </Text>
          </View>
          ...
          <View className="flex-row justify-center gap-6 pb-8">
            {!isRecording ? (
              ...
              <HapticButton
                ...
                className={cn(
                  'h-12 w-32 rounded-full bg-green-500',
                ...
                {!isReady ? (
                  ...
                  <Text className="text-lg font-medium text-white">START</Text>
                )}
              </HapticButton>
            ) : (
              ...
              <>
                <HapticButton
                  ...
                  className={cn(
                    'h-12 w-32 rounded-full border-2 border-destructive',
                    'flex-row items-center justify-center',
                ...
                <HapticButton
                  ...
                  disabled={!isRecording}
                  className={cn(
                    'h-12 w-32 rounded-full bg-destructive',
                    'flex-row items-center justify-center',
                    !isRecording && 'opacity-50'
                  )}>
      ...
      <LoaderModal isVisible={isModalVisible} title="Processing audio..." />
    </>
  );
}
