...
export default function YoutubeDetail({ note }: NoteDetailYoutubeProps) {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'youtube_summary', filter: `note_id=eq.${note.id}` },
        (payload) => {
          refetch();
        }
      )
  ...
  const VideoItem = useCallback(
    ({ video, index }: { video: YoutubeMeta; index: number }) => (
      <View className="flex-row gap-4">
        ...
        <View className="flex-1 justify-center gap-2">
          <View className="flex-col gap-1">
            ...
            <Text className="text-xs text-foreground/50">{video?.author_name}</Text>
          </View>
          <HapticButton
            hapticType="medium"
            className="items-start"
            onPress={() => handleViewTranscript(index)}>
            <View className="rounded-full bg-muted-foreground/10 p-1 px-2">
  ...
  const selectedVideoTitle = useMemo(() => {
    return (
      parsedTranscriptData[selectedVideoIndex]?.title ||
  ...
  if (isLoading) {
    return (
      <View className="mb-2 flex-1">
        <View className="w-full flex-col gap-2 rounded-lg bg-background p-4 shadow-sm shadow-black/5">
  ...
  return (
    <View className="mb-2 flex-1">
      ...
      <Modal visible={showTranscriptModal} animationType="slide" presentationStyle="fullScreen">
        <TranscriptScreen
