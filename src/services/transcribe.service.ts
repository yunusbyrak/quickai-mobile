...
const youtubeTranscribe = async (videos: TranscribeYoutubeRequest) => {
    try {
        ...
        if (error) {
            throw new Error('Failed to transcribe youtube');
        }
        return data;
    } catch (error) {
...
const textTranscribe = async (text: TranscribeTextRequest) => {
    try {
        ...
        const { data, error } = await supabase.functions.invoke('transcribe-text', {
            body: text,
            headers: {
                Authorization: `Bearer ${session.access_token}`,
            },
...
const pdfTranscribe = async (pdf: TranscribePdfRequest) => {
    try {
        ...
        const { data, error } = await supabase.functions.invoke('transcribe-pdf', {
            body: formData,
            headers: {
                Authorization: `Bearer ${session.access_token}`,
            },
